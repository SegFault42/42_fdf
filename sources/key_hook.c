...
/*   Updated: 2016/04/13 21:31:45 by rabougue         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	((t_c *)param)->pti->img_color = mlx_get_color_value(((t_c *)param)->mlx_ptr, GREEN);
	print_p(((t_c *)param)->coord, ((t_c *)param)->pti, gap, level, iso, or_x, or_y);
	mlx_put_image_to_window(((t_c *)param)->mlx_ptr, ((t_c *)param)->win_ptr, ((t_c *)param)->img_ptr, 0, 0);
}
...
void	clear_image(void *param, t_pti *img, t_p *p)
{
	...
	i = 0;
	j = 0;
	data = mlx_get_data_addr(((t_c *)param)->img_ptr, &img->bpp, &img->sizeline, &img->endian);
...
int		key_hook(int keycode, void *param, t_pti *img, t_p *p)
{
	...
	if (keycode == KEY_ESC)
	{
		mlx_destroy_image(((t_c *)param)->mlx_ptr, ((t_c *)param)->img_ptr);
		mlx_destroy_window(((t_c *)param)->mlx_ptr, ((t_c *)param)->win_ptr);
		exit(EXIT_SUCCESS);
	}
